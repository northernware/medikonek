...
export async function patientOptions(doctorId: string): Promise<PatientOption[]> {
  const patients = await prisma.patient.findMany({
    where: { household: { doctorId } },
    orderBy: [{ household: { name: "asc" } }, { lastName: "asc" }, { firstName: "asc" }],
    select: {
      id: true,
      firstName: true,
      middleName: true,
  ...
  return patients.map((p) => ({ id: p.id, label: fullName(p), householdName: p.household.name }));
}
